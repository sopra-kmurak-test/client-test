...
const QuestionDetail = ({ question, answer }) => {
  ...
  const handleQuestionEvaluate = () => {
    evaluateQuestion({
      ...
      if (response.success === 'true') {
        message.info('thumbs up success')
        setArticle(preArticle => ({
          ...preArticle,
          likeCount: preArticle.likeCount + 1
        }))
      } else {
        message.error('thumbs up failed')
      }
    })
  }
  ...
  const translateTitle = () => {
    translate({
      ...
      article.question.title = response
      setArticle(() => ({
        ...newArticle
      }))
    })
  ...
  const translateDetail = () => {
    translate({
      ...
      const newArticle = article;
      article.question.detail = response
      setArticle(() => ({
        ...newArticle
      }))
    })
          ...
          >
            <Row>
              ...
              <Col>
                ...
                <p className={styles.date}>{moment(article.question.submit_time).format('ll')}</p>
              </Col>
            </Row>
            <Row>
              <Col span={18}>
                <Popover content={() => {
                  return (
                      <>
                        <Button onClick={translateTitle} type={"text"}>Translate into German</Button>
                      </>
                  )
                }}>
            ...
            <Row>
              <Popover content={() => {
                return (
                    <>
                      <Button onClick={translateDetail} type={"text"}>Translate into German</Button>
                    </>
                )
              }}>
              ...
              return <div key={answer.answer.id}>
                <Row>
                  <Col span={3}>
                    ...
                    <p className={styles.name} style={{ fontSize: '12px', marginTop: '4px' }}>{answer.answererNickName}</p>
                  </Col>
                  ...
                  <Col span={18}>
                    <div onClick={() => { router.push(`/question/answer/${answer.answer.id}`) }} className={styles.comment}>
                      ...
                      <p>
                        <CommentOutlined />
                        <span className={styles.date} style={{ marginLeft: '4px' }}>{ answer.commentCount } comments</span>
                        <span className={styles.date} style={{ float: 'right' }}>Posted on { moment(answer.answer.answer_time).format('ll') }</span>
                      </p>
                    </div>
                  </Col>
                  ...
                  <Col>
                    <div className={styles.hover} onClick={() => handleEvaluate(answer.answer.id)}>
                      <LikeTwoTone />
                      <span style={{ marginLeft: '4px' }} className={styles.date}>{ answer.likeCount } likes</span>
                    </div>
                  </Col>
                </Row>
